  ...
  <script>
    ...
    <nav id='menu'>
        ...
        <ul class="MenuUl">
          ...
          <div class="MenuHeading">
            <li><a href="Index.php#targetShop">Shop</a></li>
          </div>
          <div class="MenuHeading">
    ...
    if ($row) {
        ...
        if (count($coupon_ids) > 0) {
            ...
            while ($coupon_row = mysqli_fetch_assoc($coupon_data_result)) {
                echo '        
                <div class="item" id="' . $coupon_row["Coupon_id"] . '">
                    <img src="' . $coupon_row["image_src"] . '" alt="' . $coupon_row["brand"] . '" class="item-image">
                    <div class="item-name col">
                        <p>' . $coupon_row["brand"] . '</p>
                        <p>' . $coupon_row["coupon_desc"] . '</p>
                    </div>
                    <div class="item-quantity">
                        Pack of : ' . $coupon_row["quantity"] . '
                    </div>
                    <div class="item-price">₹' . $coupon_row["price"] . '</div>
                    <div class="item-remove"><img src="images used\remove.png" alt="X" onclick="removeFromCart()"></div>
                </div>';
            }
        }
    } else {
...
?>
    ...
    <?php
        if (count($coupon_ids) === 0) {
            echo '<div class="emptyCart"style=" height: 80vh; display: flex; align-items: center; justify-content: center;">
    ...
    function removeFromCart(linkElement) {
        // Find the closest parent with class 'item'
        var parentDiv = linkElement.closest('.item');
        alert('Worked');
